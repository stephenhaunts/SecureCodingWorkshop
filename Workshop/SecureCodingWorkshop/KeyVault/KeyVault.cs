...
namespace SecureCodingWorkshop.KeyVault
{
    public class KeyVault : KeyVaultBase, IKeyVault
    {
		...
		public async Task<byte[]> UnwrapSymmetricKeyAsync(string keyId, byte[] wrappedSymmetricKey)
		{
			if (string.IsNullOrEmpty(keyId))
			{
				throw new ArgumentNullException(keyId, "Key NickName is Null.");
			...
			var unwrappedKey = await KeyVaultClient.UnwrapKeyAsync(keyId, JsonWebKeyEncryptionAlgorithm.RSAOAEP, wrappedSymmetricKey);
